...
func NewBedrock(modelProvider, modelID string) *Bedrock {
	return &Bedrock{
...
func (a *Bedrock) Call(ai domain.Charactor, histories []domain.History, userMessage string) (string, error) {

	llm, err := bedrock.New(
		bedrock.WithModelProvider(a.modelProvider),
		bedrock.WithModel(a.modelID),
	)
	if err != nil {
		return "", err
	}
...
func (a *Bedrock) convertHistory(ctx context.Context, histories []domain.History) *memory.ChatMessageHistory {
